...
$(document).ready(function() {
	...
	var characters = {
		...
		'Voldermort': {
			...
			health: 150,
			attack: 8,
			imageUrl: "assets/images/voldermort.png",
			enemyAttackBack: 20
		}, 
		'Dementor': {
			name: 'Dementor',
			health: 180,
			attack: 7,
			imageUrl: "assets/images/dementor.jpg",
			enemyAttackBack: 20
		}
	};
	...
	var renderOne = function(character, renderArea, makeChar) {
	  ...
	  $("#attack-button").on("click", function() {
		...
		if ($('#defender').children().length !== 0) {
		  ...
		  var attackMessage = "You used the Sectumsempra spell on " + currDefender.name + " for " + (currSelectedCharacter.attack * turnCounter) + " damage.";
		  renderMessage("clearMessage");
		  //combat
		  ...
		  if (currDefender.health > 0) {
			...
			//player state change
			var counterAttackMessage = currDefender.name + " countered with the Expelliarmus spell for " + currDefender.enemyAttackBack + " damage.";
